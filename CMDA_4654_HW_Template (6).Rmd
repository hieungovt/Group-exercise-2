...
final_unw <- mykNN(train = train_scaled, test = test_scaled, 
                   y_train = train_response, y_test = test_response, 
                   k = best_k_unweighted, weighted = FALSE)
final_w <- mykNN(train = train_scaled, test = test_scaled, 
                 y_train = train_response, y_test = test_response, 
...
k_ <- mykNN(train = train_scaled, test = test_scaled, 
               y_train = train_response, y_test = test_response, 
               k = 5, weighted = TRUE)
k_10 <- mykNN(train = train_scaled, test = test_scaled, 
                y_train = train_response, y_test = test_response, 
                k = 10, weighted = TRUE)
...
my_knn_result <- mykNN(train = train_scaled, test = test_scaled, 
                       y_train = train_response, y_test = test_response, 
                       k = best_k_unweighted, weighted = FALSE)

# Use the built-in knn() function for unweighted kNN.
builtin_knn <- knn(train = train_scaled, test = test_scaled, 
...
for(i in seq_along(k_set)){
  ...
  fit <- mykNN(train = train_temp,
               ...
               k = k_val,
               weighted = TRUE)
  results_a$MSE[i] <- fit$MSE
}

# Display the results table 
kable(results_a, caption = "Test MSE for dwkNN (ozone ~ temperature) for various k")

# Choose the best k for plotting the regression line.
best_k_a <- results_a$k[which.min(results_a$MSE)]
cat("Best k in part (a):", best_k_a, "with MSE =", min(results_a$MSE), "\n")
